...
public class BoardHelper {
    ...
    public static int[][] getNewBoardAfterMove(int[][] board, Move move, int player) {
        ...
        // place piece
        newboard[move.getRow()][move.getCol()] = player;
        // reverse pieces
        ArrayList<Point> rev = BoardHelper.getReversePoints(newboard, player, move.getRow(), move.getCol());
        for (Point pt : rev) {
            ...
            for (int j = 0; j < 8; j++) {
                if (board[i][j] == oplayer) {
                    ...
                    if (i > 0 && board[i - 1][j] == 0)
                        possiblefrontiers.add(new Point(i - 1, j));
                    // down
                    if (i < 7 && board[i + 1][j] == 0)
                        possiblefrontiers.add(new Point(i + 1, j));
                    // right
                    if (j < 7 && board[i][j + 1] == 0)
                        possiblefrontiers.add(new Point(i, j + 1));
                    // left
                    if (j > 0 && board[i][j - 1] == 0)
                        possiblefrontiers.add(new Point(i, j - 1));
                    // up-left
                    if (i > 0 && j > 0 && board[i - 1][j - 1] == 0)
                        possiblefrontiers.add(new Point(i - 1, j - 1));
                    // up-right
